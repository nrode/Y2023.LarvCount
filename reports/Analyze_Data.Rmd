...
## +Incubation
m13 <- lme4::lmer(log(Time_Sec)~Incubation*True_No_Larvae+Round+(1|Sample_ID)+(1|Observer_ID), data=data)
summary(m13)
...
p2A <- MSEdataSample_IDReplicateIncubation %>%
  ...
  xlab("Number of larvae") + ylab("Time for counting larvae (sec.)") + scale_color_discrete(name = "Replicate") +
...
p2B <- MSEdataSample_IDReplicateIncubation %>%
  dplyr::mutate(True_No_Larvae=dplyr::if_else(Replicate==1, True_No_Larvae-x, True_No_Larvae+x)) %>%
  ggplot(aes(x=True_No_Larvae, y=Bias_Obs_No_Larvae, color=Replicate, shape=Incubation)) + geom_point() + xlim(-1, 35) + ylim(-20, 1) + xlab("Number of larvae") + ylab("Bias in larval count estimates") + scale_color_discrete(name = "Replicate") + theme(text = element_text(size = siz))+ 
...
p2C <- MSEdataSample_IDReplicateIncubation %>%
  dplyr::mutate(True_No_Larvae=dplyr::if_else(Replicate==1, True_No_Larvae-x, True_No_Larvae+x)) %>%
  ggplot(aes(x=True_No_Larvae, y=SD_Obs_No_Larvae, color=Replicate, shape=Incubation)) + geom_point() + xlim(-1, 35) + ylim(0, 10) + xlab("Number of larvae") + ylab("Standard deviation \nin larval count estimates") + scale_color_discrete(name = "Replicate") + theme(text = element_text(size = siz))+ 
...
## Relationship between bias and expected number of larvae
p2B <- MSEdataSample_IDReplicateIncubation %>%
  dplyr::filter(True_No_Larvae<6) %>%
  ggplot(aes(x=Mean_Time_Sec, y=Bias_Obs_No_Larvae, shape=Incubation)) + geom_point() + xlim(0, 30) + ylim(-2.5, 1) + xlab("Time for counting larvae (sec.)") + ylab("Bias in larval count estimates") + scale_color_discrete(name = "Replicate") + theme(text = element_text(size = siz))
