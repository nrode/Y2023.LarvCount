...
```{r}
# counting mean time/ sample when incubated or not incubated 
standard_error<-function(x) {
  sd(x)/sqrt(length(x))
}
data %>%
  dplyr::mutate(Counting_error = abs(Obs_No_Larvae - True_No_Larvae), Category_True_No_Larvae=cut(True_No_Larvae, breaks=c(0, 5, 10, 15, Inf), labels=c("0-5","5-10","10-15","+15"), right = FALSE))%>%
  ...
  dplyr::summarise(Counting_error_mean=mean(Counting_error),
                   Time_Sec_mean=mean(Time_Sec), ST_error = standard_error(Time_Sec))
```
